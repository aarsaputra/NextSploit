...
class ScanReport:
    ...
    def _save_html(self, filepath: str):
        ...
        for v in data.get("vulnerability_matrix", []):
            if "VULNERABLE" in v["status"]:
                status_class = "vuln"
            elif "NOT_APPLICABLE" in v["status"]:
                status_class = "na"
            elif "INCONCLUSIVE" in v["status"]:
                status_class = "incon"
            else:
                status_class = "safe"
            matrix_rows += f"""
        ...
        for mod in data["module_results"]:
            ...
            if mod["status"] == "VULNERABLE":
                status_class = "vuln"
            elif mod["status"] == "NOT_APPLICABLE":
                status_class = "na"
            elif mod["status"] == "INCONCLUSIVE":
                status_class = "incon"
            else:
                status_class = "safe"
...
<head>
    ...
    <style>
        ...
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        ...
        /* Summary Cards */
        .summary-grid {{ display: grid; grid-template-columns: repeat(auto-fit, minmax(130px, 1fr)); gap: 1rem; margin: 1rem 0; }}
        .summary-card {{
        ...
        .summary-card.na .number {{ color: var(--text-dim); }}
        .summary-card.incon .number {{ color: var(--yellow); }}
        ...
        td.na {{ color: var(--text-dim); }}
        td.incon {{ color: var(--yellow); font-weight: bold; }}
        ...
        .status-badge.na {{ background: rgba(139,148,158,0.2); color: var(--text-dim); }}
        .status-badge.incon {{ background: rgba(210,153,34,0.2); color: var(--yellow); }}
...
<body>
    ...
    <div class="summary-grid">
        ...
        <div class="summary-card na"><div class="number">{summary['not_applicable']}</div><div class="label">Not Applicable</div></div>
        <div class="summary-card incon"><div class="number">{summary['inconclusive']}</div><div class="label">Inconclusive</div></div>
    ...
    <footer>
        Generated by {data['tool']} v{data['version']} — Next.js Vulnerability Scanner & Exploit Tool
    </footer>
</body>
</html>"""
